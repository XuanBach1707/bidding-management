...
export const ResultSummaryBadge = ({ hsmtId }: ResultSummaryBadgeProps) => {
  ...
  return (
    <div className="mt-2 md:mt-3 bg-gradient-to-br from-emerald-50 to-white border border-emerald-100 rounded-lg p-2.5 md:p-3 relative overflow-hidden group shadow-sm animate-in fade-in zoom-in-95 duration-300">
      {/* Background Icon */}
      ...
      <div className="flex items-center gap-2 mb-1.5 md:mb-2">
        ...
        <span className="text-[9px] md:text-[10px] font-extrabold text-emerald-700 uppercase tracking-widest truncate">
      ...
      <div className="space-y-1 pl-0.5 relative z-10">
        <div className="text-xs md:text-sm font-bold text-slate-800 line-clamp-2 leading-snug" title={data.winnerName || "Chưa có tên nhà thầu"}>
            {data.winnerName || "---"}
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between text-xs pt-1 gap-2">
            ...
            <div className="flex items-center gap-1 text-slate-400" title="Ngày phê duyệt">
                ...
                <span className="text-[10px] md:text-xs">{data.approvalDate ? new Date(data.approvalDate).toLocaleDateString('vi-VN') : "--/--"}</span>
            </div>
        </div>
      </div>
    </div>
