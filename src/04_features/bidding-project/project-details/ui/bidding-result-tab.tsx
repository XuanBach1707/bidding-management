...
import { Loader2, FileText, CheckCircle2, XCircle, Package, CalendarCheck, AlertCircle } from "lucide-react";

// Import Entity & Shared
import { biddingResultApi, BidderWinner, BidderFailed } from "@/entities/bidding-result";
...
interface GroupedBidder {
    key: string;            // Key định danh (Tên liên danh hoặc ID nhà thầu độc lập)
    isJointVenture: boolean;
    groupName: string;      // Tên hiển thị của nhóm (VD: Liên danh PC1)
    ...
    // Các thông tin chung của cả nhóm
    price?: string | null;
    evaluatedPrice?: string | null;
    reason?: string | null;
    technicalScore?: string | null;
    contractPeriod?: string | null;
    role?: string | null; // Lưu role gốc để hiển thị
}
...
export const BiddingResultTab = ({ hsmtId }: BiddingResultTabProps) => {
  ...
  const groupedWinners = useMemo(() => {
    ...
    data.winners.forEach((w) => {
        // [QUAN TRỌNG] User xác nhận: field 'role' của winner chứa tên Liên danh
        // Nếu role có chữ "Liên danh" -> Gom nhóm. Nếu không -> Độc lập.
        ...
        if (!groups[groupKey]) {
            groups[groupKey] = {
                ...
                role: w.role // Giữ lại để hiển thị
            };
        }
  ...
  const groupedFailed = useMemo(() => {
    ...
    data.failedBidders.forEach((f) => {
        // Failed bidder dùng trường jointVentureName
        ...
        if (!groups[groupKey]) {
            groups[groupKey] = {
                ...
                members: [],
                reason: f.reason, // Lý do thường giống nhau cho cả liên danh
            };
        }
    ...
    <div className="space-y-8 pb-20 max-w-6xl mx-auto">
      ...
      <Card className="overflow-hidden border-slate-200 shadow-sm">
         <div className="bg-slate-50/80 px-6 py-4 border-b border-slate-100 flex items-center gap-2">
            ...
            <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wide">Thông tin Quyết định phê duyệt</h3>
         </div>
         
         <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-6 gap-x-8">
            <div className="space-y-1">
                <div className="text-[11px] text-slate-500 font-bold uppercase tracking-wider">Số quyết định</div>
                <div className="font-semibold text-slate-900 text-sm border-b border-slate-100 pb-1">{data.decisionNumber || "---"}</div>
            </div>
            <div className="space-y-1 lg:col-span-2">
                <div className="text-[11px] text-slate-500 font-bold uppercase tracking-wider">Cơ quan ban hành</div>
                <div className="font-semibold text-slate-900 text-sm border-b border-slate-100 pb-1" title={data.approvingAgency || ""}>
                    {data.approvingAgency || "---"}
                </div>
            </div>

            <div className="space-y-1">
                <div className="text-[11px] text-slate-500 font-bold uppercase tracking-wider">Ngày phê duyệt</div>
            ...
            <div className="space-y-1">
                <div className="text-[11px] text-slate-500 font-bold uppercase tracking-wider">Ngày đăng tải</div>
                <div className="font-medium text-slate-900 text-sm flex items-center gap-2">
                    <CalendarCheck className="w-3.5 h-3.5 text-slate-400" />
            ...
            <div className="space-y-1">
                <div className="text-[11px] text-slate-500 font-bold uppercase tracking-wider">Trạng thái KQ</div>
                <div>
            ...
            <div className="space-y-1">
                <div className="text-[11px] text-slate-500 font-bold uppercase tracking-wider">Giá gói thầu</div>
      ...
      {/* 2. BẢNG DANH SÁCH NHÀ THẦU (ĐÃ GOM NHÓM) */}
      <Card className="overflow-hidden border-slate-200 shadow-sm">
         <div className="bg-slate-50/80 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
            ...
            {/* Logic đếm số lượng thực tế nhà thầu tham gia */}
            <div className="text-xs font-medium text-slate-500">
                Tổng số: <span className="font-bold text-slate-900">{(data.winners?.length || 0) + (data.failedBidders?.length || 0)}</span> nhà thầu
            </div>
         </div>
                ...
                {groupedWinners.map((group, index) => (
                    <TableRow key={`group-win-${index}`} className="bg-emerald-50/40 hover:bg-emerald-50/60 transition-colors">
                        ...
                        <TableCell className="align-top py-4">
                            ...
                            {/* Thông tin phụ: Thời gian thực hiện */}
                ...
                {groupedFailed.map((group, index) => (
                    <TableRow key={`group-fail-${index}`} className="hover:bg-slate-50/50">
                        ...
                        <TableCell className="align-top py-4">
                            ...
                            {/* Lý do trượt (Hiển thị 1 lần cho cả nhóm) */}
                            {group.reason && (
