      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-4 md:gap-0">
        <div className="space-y-1">
          <div className="flex flex-wrap items-center gap-2 md:gap-3">
            <h1 className="text-lg md:text-2xl font-bold text-slate-900 leading-snug">{project.name}</h1>
            <Badge variant="outline" className="bg-blue-50 text-blue-600 border-blue-200 uppercase font-bold text-[10px] h-6">
              {project.status}
            </Badge>
          </div>
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <Button variant="outline" size="sm" className="font-bold flex-1 md:flex-none">Chỉnh sửa</Button>
          <Button size="sm" className="bg-blue-600 font-bold shadow-blue-100 shadow-lg flex-1 md:flex-none">Gửi duyệt</Button>
        </div>
      </div>

      {/* Grid: 1 cột (Mobile) -> 3 cột (PC) */}
