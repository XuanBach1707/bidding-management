...
export const PolicyListTable: React.FC<PolicyListTableProps> = ({ onEdit }) => {
  ...
  const handleDelete = async (id: number, name: string) => {
    // Tạm thời dùng confirm để nhanh, hoặc bạn có thể thay bằng shadcn AlertDialog
    if (confirm(`Bạn có chắc chắn muốn xóa chính sách "${name}"?`)) {
      try {
        await abacApi.deletePolicy(id);
        toast({
          title: "Đã xóa",
          description: `Chính sách "${name}" đã được gỡ bỏ.`,
        });
        fetchPolicies(); 
      } catch (error: any) {
        toast({
  ...
  if (loading) {
    return <div className="p-4 text-center text-gray-500">Đang tải danh sách chính sách...</div>;
  }
    ...
    <div className="bg-white rounded-lg shadow border overflow-hidden">
      <table className="w-full text-left border-collapse">
        <thead className="bg-gray-50 text-gray-600 uppercase text-xs font-semibold">
          <tr>
            <th className="p-4 border-b w-20">Priority</th>
            ...
            policies.map((policy) => (
              <tr key={policy.id} className="hover:bg-gray-50 border-b last:border-b-0 transition-colors">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex gap-1 flex-wrap">
                      {policy.action.map((act) => (
                        <span key={act} className="text-[10px] border border-gray-200 px-1.5 rounded bg-white text-gray-600 uppercase">{act}</span>
                      ))}
                    </div>
                  </div>
                </td>
                ...
                <td className="p-4 text-center">
                  <span className={`w-2.5 h-2.5 inline-block rounded-full ${policy.is_active ? 'bg-green-500' : 'bg-gray-300'}`}></span>
                </td>
                <td className="p-4 text-right">
                  <button onClick={() => onEdit(policy)} className="text-blue-600 hover:text-blue-800 text-xs font-bold mr-4">Sửa</button>
                  <button onClick={() => policy.id && handleDelete(policy.id, policy.name)} className="text-red-500 hover:text-red-700 text-xs font-bold">Xóa</button>
                </td>
              </tr>
            ))
          )}
