...
import { Activity, RefreshCcw, Search, Eye, AlertCircle, CheckCircle2, Clock, Terminal, Calendar } from "lucide-react"
...
const StatusBadge = ({ status }: { status: CrawlerLogStatus }) => {
  switch (status) {
    case "SUCCESS":
      return (
        <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-50 font-semibold gap-1 pr-3">
          <CheckCircle2 size={12} /> SUCCESS
        </Badge>
      )
    case "FAILED":
      return (
        <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200 hover:bg-red-50 font-semibold gap-1 pr-3">
          <AlertCircle size={12} /> FAILED
        </Badge>
      )
    case "RUNNING":
      return (
        <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-50 font-semibold gap-1 pr-3">
...
function LogDetailDialog({ 
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[85vh] flex flex-col p-0 gap-0 overflow-hidden bg-slate-50">
        
        {/* HEADER */}
        <div className="p-6 border-b border-slate-200 bg-white">
            <DialogHeader>
            <DialogTitle className="flex items-center gap-3 text-xl font-bold text-slate-800">
                <Terminal className="w-6 h-6 text-[#009d98]" />
                Chi tiết Nhật ký <span className="font-mono text-slate-400 font-normal">#{logId}</span>
            </DialogTitle>
            <DialogDescription>
                Xem lại thông số cấu hình và kết quả của phiên chạy này.
            </DialogDescription>
            </DialogHeader>
        </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-4 gap-4">
                 <Card className="shadow-sm border-slate-200 bg-white">
                    <CardContent className="p-4 flex flex-col items-center justify-center text-center">
                       <span className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Trạng thái</span>
                       <StatusBadge status={logDetail.status} />
                    </CardContent>
                 </Card>
                 <Card className="shadow-sm border-slate-200 bg-white">
                    <CardContent className="p-4 flex flex-col items-center justify-center text-center">
                       <span className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Kết quả</span>
                       <span className="text-xl font-extrabold text-[#009d98] tabular-nums">{logDetail.packagesFound}</span>
                 ...
                 <Card className="col-span-2 shadow-sm border-slate-200 bg-white">
                    <CardContent className="p-4 flex items-center justify-around h-full">
                       <div className="flex flex-col">
                          <span className="text-xs text-slate-400 flex items-center gap-1"><Clock size={12} /> Bắt đầu</span>
                          <span className="text-sm font-mono font-medium text-slate-700">{format(new Date(logDetail.startTime), "HH:mm:ss dd/MM/yyyy")}</span>
                       </div>
                       <div className="h-8 w-[1px] bg-slate-100"></div>
                       <div className="flex flex-col">
                          <span className="text-xs text-slate-400 flex items-center gap-1"><CheckCircle2 size={12} /> Kết thúc</span>
                          <span className="text-sm font-mono font-medium text-slate-700">{format(new Date(logDetail.endTime), "HH:mm:ss dd/MM/yyyy")}</span>
                       </div>
                    </CardContent>
                 </Card>
              </div>
              ...
              {logDetail.errorMessage && (
                <div className="bg-red-50 border border-red-200 p-4 rounded-lg flex gap-3 items-start">
                  ...
                  <div>
                     ...
                     <p className="text-sm text-red-600 mt-1 font-mono bg-white/50 p-2 rounded border border-red-100">{logDetail.errorMessage}</p>
                  </div>
                </div>
              )}
              ...
              <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                <div className="px-4 py-3 bg-slate-50 border-b border-slate-200 flex items-center gap-2">
                   ...
                   <span className="font-bold text-sm text-slate-700">Snapshot Cấu hình (Tại thời điểm chạy)</span>
                </div>
                <div className="p-5 space-y-4">
                   <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-1">
                         <label className="text-xs font-bold text-slate-400 uppercase">Tên luật</label>
                         <p className="font-semibold text-slate-800">{logDetail.rule.ruleName}</p>
                      </div>
                      <div className="space-y-1">
                         <label className="text-xs font-bold text-slate-400 uppercase">Lĩnh vực</label>
                   ...
                   <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-1">
                         <label className="text-xs font-bold text-slate-400 uppercase">Từ khóa (Include)</label>
                         <div className="flex flex-wrap gap-1.5 mt-1">
                            {logDetail.rule.keywordsInclude.length > 0 ? (
                                logDetail.rule.keywordsInclude.map((k: string, i: number) => (
                                    <span key={i} className="px-2 py-0.5 bg-slate-100 border border-slate-200 rounded text-xs font-medium text-slate-600">{k}</span>
                                ))
                            ) : <span className="text-xs text-slate-400 italic">Không có</span>}
                         </div>
                      </div>
                      <div className="space-y-1">
                         <label className="text-xs font-bold text-slate-400 uppercase">Khu vực</label>
                         <div className="flex flex-wrap gap-1.5 mt-1">
                   ...
                   <div className="space-y-1">
                         <label className="text-xs font-bold text-slate-400 uppercase">Ngân sách</label>
...
export function SystemLogs() {
  ...
  return (
    <Card className="h-full flex flex-col border-slate-200 shadow-sm bg-white">
      <CardHeader className="pb-0 border-b border-slate-50 bg-slate-50/50 pt-5">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
            <div>
                ...
                <CardDescription className="mt-1">
                    Lịch sử hoạt động của Bot Crawler theo thời gian thực.
                </CardDescription>
            </div>
            ...
            <div className="flex gap-3">
                ...
                <Button 
                  ...
                  className="h-9 border-slate-200 hover:text-[#009d98] hover:border-[#009d98]"
                >
            ...
            <Table>
              <TableHeader className="sticky top-0 bg-slate-50 z-10 shadow-sm">
                <TableRow className="border-slate-200 hover:bg-slate-50">
                  ...
                  <TableHead className="w-[200px] font-bold text-slate-700">Thời gian</TableHead>
                  ...
                  logs.map((log) => (
                    ...
                    >
                      ...
                      <TableCell>
                         ...
                         {/* Nếu running thì chưa có end time */}
                         {log.status !== 'RUNNING' && (
                             <span className="text-[10px] text-slate-400 mt-0.5 block">
                                Duration: {log.endTime ? ((new Date(log.endTime).getTime() - new Date(log.startTime).getTime()) / 1000).toFixed(1) : 0}s
                             </span>
                         )}
                      </TableCell>
                      ...
                      <TableCell>
                        ...
                        {/* --- [ĐÃ SỬA] Đã bỏ dòng log.sourceUrl ở đây --- */}
                      </TableCell>
