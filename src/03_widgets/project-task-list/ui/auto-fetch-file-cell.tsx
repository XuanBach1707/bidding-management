...
export const AutoFetchFileCell = ({ taskName }: Props) => {
  ...
  if (files.length > 0) {
    ...
    return (
      // [UPDATE] max-w linh hoạt hơn: Mobile full width, Desktop max 200px
      <div className="group flex items-center gap-2 w-full md:max-w-[200px] overflow-hidden">
        ...
        <div className="flex flex-col min-w-0 flex-1">
           ...
           {moreCount > 0 && (
             <span className="text-[9px] text-slate-400 font-medium truncate">+{moreCount} file khác</span>
           )}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex items-center gap-2 opacity-40">
       ...
       <span className="text-[10px] italic text-slate-400 font-medium whitespace-nowrap">-- Chưa có --</span>
    </div>
  );
};