...
import { CheckSquare, Clock, AlertCircle, Edit3, ListTodo } from "lucide-react"; 
...
import { TASK_PRIORITY_CONFIG } from "@/entities/task/model/constants";
...
export const DailyTasksWidget = () => {
  ...
  const getFilterBtnClass = (active: boolean) => 
    cn(
      "text-xs font-bold px-3 py-1.5 rounded-lg border transition-all uppercase tracking-tight whitespace-nowrap flex-shrink-0",
      active 
        ? "bg-[#009d98]/10 text-[#009d98] border-[#009d98]/30 shadow-sm" 
        : "bg-white text-slate-500 border-slate-200 hover:bg-slate-50 hover:text-slate-700"
    );

  if (loading) return <Skeleton className="h-[500px] md:h-[600px] w-full rounded-xl" />;

  return (
    // Mobile: h-[500px]. PC: h-[600px]
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col h-[500px] md:h-[600px] overflow-hidden">
      {/* --- HEADER --- */}
      <div className="p-4 md:p-5 border-b border-slate-100 bg-white shrink-0">
        {/* Title Section */}
        <div className="flex flex-col gap-1 mb-4">
            ...
            {/* Mobile: pl-0 để căn lề trái. PC: pl-10 thụt vào cho đẹp */}
            <p className="text-sm text-slate-500 font-medium pl-0 md:pl-10">
        ...
        {/* Filters - Mobile Optimized: Scroll ngang */}
        {/* -mx-4 px-4: Hack để scroll tràn ra mép màn hình điện thoại nhưng content vẫn căn lề */}
        <div className="overflow-x-auto -mx-4 px-4 md:mx-0 md:px-0 pb-1 no-scrollbar">
            <div className="flex gap-2 p-1 bg-slate-50 rounded-lg w-max md:w-fit">
             ...
             filteredTasks.map((task) => {
                ...
                return (
                  <div 
                    key={task.id}
                    className={cn(
                      "group flex items-start gap-3 md:gap-4 p-3 md:p-4 bg-white border rounded-xl transition-all cursor-pointer relative overflow-hidden",
                      isDone 
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-1">
                        {/* Title: line-clamp-2 để tránh tên dài quá làm vỡ layout mobile */}
                        <h4 className={cn(
                          "text-sm font-bold transition-colors pr-2 line-clamp-2",
                          isDone ? "text-slate-500 line-through" : "text-slate-800 group-hover:text-[#009d98]"
                        )}>
                          {task.taskName}
                        </h4>
                        
                        {/* Actions: PC thì hover mới hiện. Mobile thì ẩn luôn để đỡ rối (Click vào task để edit) */}
                        {!isDone && (
                      ...
                      <div className="flex flex-wrap items-center gap-2 md:gap-3 mt-2">
                          ...
                          {!isDone && (
                            <span className={cn(
                              "text-[10px] font-bold px-2 py-0.5 rounded border flex items-center gap-1 uppercase whitespace-nowrap",
                              task.priority === "HIGH" 
                          ...
                          {deadlineDate && (
                            <span className={cn(
                              "text-[10px] font-bold flex items-center gap-1 whitespace-nowrap",
