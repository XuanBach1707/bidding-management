...
interface WasmModule {
  ...
  compress_image: (data: Uint8Array) => Uint8Array<ArrayBuffer>;
  export_to_docx: (html: string) => Uint8Array<ArrayBuffer>;
  read_docx_to_html: (data: Uint8Array) => string;
  sanitize_html_paste: (html: string) => string;
}
...
`);
  ...
  useEffect(() => {
    const loadWasm = async () => {
      try {
        // @ts-expect-error: module generated by wasm-pack, not available until `wasm-pack build` is run
      ...
      <section className="border rounded-lg p-6 bg-white shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-8">
            <div>
                ...
                {imgOriginal && <img src={imgOriginal} alt="Ảnh gốc" className="w-full h-48 object-contain border bg-slate-50" />}
            </div>
            <div>
                ...
                {/* eslint-disable-next-line @next/next/no-img-element */}
                {imgCompressed && <img src={imgCompressed} alt="Ảnh sau khi nén" className="w-full h-48 object-contain border bg-slate-50" />}
            </div>
        </div>
      </section>
